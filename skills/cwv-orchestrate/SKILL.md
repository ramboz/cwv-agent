---
name: cwv-orchestrate
description: "Long-running autonomous loop that races CWV intervention candidates against a target URL, measures each with `launcher.js`, verdicts each with `oracle.js`, and **additively** promotes validated patches — with the user as final reviewer."
---

<!-- Generated from .agents/skills/cwv-orchestrate.md by plugin-sync.js — edit there, then `npm run plugin:sync`. -->
