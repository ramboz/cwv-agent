...
forbidden_techniques:
  ...
  - pattern: 'dynamicmedia/deliver/\S*[?&](?:wid|hei)='
    reason: "The DM delivery API (/adobe/dynamicmedia/deliver/dm-aid--…) ignores classic Scene7 wid/hei — use width/height. wid= returns the full-size original (zero bytes saved) and only changes the URL, which can regress LCP via a cold-cache origin fetch. For LCP image-weight reduction see lcp-image.md."
...
> **Risk tier:** low · **Applies to:** EDS, CS, AMS, Headless · **CWV metric:** CLS
...
> **Scope — this playbook is CLS (layout reservation), not LCP weight.** To cut an LCP hero's transfer *bytes* via Dynamic Media / EDS resize params (use `width`, **never** the classic Scene7 `wid` on a `/adobe/dynamicmedia/deliver/` URL), see [`lcp-image.md`](./lcp-image.md) → *Reduce LCP image transfer size*.

## When to apply / when to skip

**Apply when:**
- Image is rendered in HTML/HTL markup, **or is created by project JavaScript** (e.g. EDS `createOptimizedPicture`, image block decorators, server-side rendered SPAs) — anywhere the markup-emitting code lives in this repo
...
> inside a `<picture>`** (the EDS `createOptimizedPicture` default, and many CS components) is
...
### EDS: setting attributes in `createOptimizedPicture`

When the markup is generated by EDS's image-block decoration logic (or the project's `createOptimizedPicture` helper in `aem.js`), the fix is one line in the helper rather than per-image markup edits:
...
This is different from JS that **emits** image markup with correct attributes from the start (e.g. EDS's `createOptimizedPicture`, block decorators, SSR'd SPAs). Those are fine — see the "EDS: setting attributes in `createOptimizedPicture`" recommended approach above.

## Flavor-specific notes

### AMS

JSP templates may have complex output logic — verify the actual rendered HTML (not just the template source) before modifying. Some `<cq:include>` paths produce output that bypasses the obvious template file.
