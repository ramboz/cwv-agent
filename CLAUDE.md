...
**New contributors** - read in order for orientation:

1. [Executive Summary](.claude/executive-summary.md) - **Start here** for the high-level "what and why"
2. [Architecture](.claude/architecture.md) - Core system design

For in-flight work and open decisions, see **Current work** below.

---

## Core Architecture

Durable references — what the system is.
...
Task state — what's pending or in flight. These docs change frequently and should be skimmed at the start of a session, not memorized.
...
`context-*.md` files are domain-scoped guardrails — anti-patterns and rules of thumb to load when working in that area, not on every session.

| Document | Load when working on... |
|----------|-------------------------|
| [context-testing.md](.claude/context-testing.md) | Tests, fixtures, validation patterns |
| [context-css-optimization.md](.claude/context-css-optimization.md) | CSS loading, render-blocking, critical path |

CMS-specific guardrails live alongside the agents that use them in `src/prompts/contexts/`.

### Path-Scoped Rules (auto-loaded)

Rules in `.claude/rules/` load automatically when editing files that match their `paths:` glob — no manual loading needed.

| Rule | Paths | What it enforces |
|------|-------|-----------------|
| [rule-modules.md](.claude/rules/rule-modules.md) | `src/rules/**` | `evaluate()` contract, return shape, no hardcoded thresholds |
| [prompt-construction.md](.claude/rules/prompt-construction.md) | `src/prompts/**` | Template system usage, shared fragments, CMS context placement |
| [schemas.md](.claude/rules/schemas.md) | `src/core/multi-agents/**` | Zod/Gemini compatibility (no `$ref`, no unions, `jsonSchema` method) |
...
These paths are generated, ephemeral, or sensitive. Do not modify them programmatically.

- `.cache/` - Local analysis cache populated by collectors (regenerated on demand)
- `coverage/` - Test coverage output (regenerated by the test runner)
- `package-lock.json` - Managed by `npm install`; do not hand-edit
- `.env`, `credentials*.json` - Secrets; never commit, never rewrite from a script
